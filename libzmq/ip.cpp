...
void zmq::unblock_socket (fd_t s_)
{
    ...
    errno_assert (rc != -1);
#else
    //printf("ip.cpp unblock_socket fd_t=%d\n", s_);
    int flags = fcntl (s_, F_GETFL, 0);
    //printf("ip.cpp unblock_socket fcntl done flag=%d\n", flags);
    if (flags == -1)
        flags = 0;
    int rc = fcntl (s_, F_SETFL, flags | O_NONBLOCK);
    //printf("ip.cpp unblock_socket set to NONBLOCK rc = %d \n", rc);
    errno_assert (rc != -1);
#endif
}
...
int zmq::make_fdpair (fd_t *r_, fd_t *w_)
{
    ...
    printf("ip.cpp make_fdpair eventfd=%d\n", fd); 
    if (fd == -1) {
...
try_tcpip:
    ...
    *r_ = accept (listener, NULL, NULL);
    ...
    int listener = open_socket (AF_INET, SOCK_STREAM, IPPROTO_TCP);
    errno_assert (listener != -1);

    int rc;

    int on = 1;
    //rc = setsockopt (listener, IPPROTO_TCP, TCP_NODELAY, (char *) &on, sizeof(on));
    //errno_assert (rc != -1);
    
    rc = bind (listener, (struct sockaddr *) &lcladdr, sizeof (lcladdr));
    errno_assert (rc != -1);
    printf("ip.cpp make_fdpair bind done! rc=%d\n",rc);
    socklen_t lcladdr_len = sizeof (lcladdr);
    ...
    rc = listen (listener, 1);
    printf("ip.cpp make_fdpair listen done! rc=%d\n",rc);
    errno_assert (rc != -1);

    *w_ = open_socket (AF_INET, SOCK_STREAM, IPPROTO_TCP);
    errno_assert (*w_ != -1);
    ...
    *r_ = accept (listener, NULL, NULL);
    printf("ip.cpp make_fdpair accept done! r_=%d\n", *r_);
    errno_assert (*r_ != -1);
