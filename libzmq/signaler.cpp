...
zmq::signaler_t::signaler_t ()
{
    ...
    int makeRet = make_fdpair (&_r, &_w);
    printf("signaler.c signaler_t constructor, unblock r,w FD ret=%d\n", makeRet);
    if (makeRet == 0) {
        unblock_socket (_w);
        unblock_socket (_r);
    }
...
void zmq::signaler_t::send ()
{
    ...
    //printf("signaler.c send inc=%d at 0x%x\n", inc, &inc);
    //printf("signaler.c send write _w=%d [0x%x]\n", _w, &_w);
    ssize_t sz = write (_w, &inc, sizeof (inc));
    printf("signaler.c write inc[0x%x]=%d with size=%d bytes as sz=%d bytes to _w=%d\n", &inc, inc, sizeof(inc), sz, _w);
...
int zmq::signaler_t::wait (int timeout_) const
{
    ...
    if (unlikely (pid != getpid ())) {
        ...
        // response.
        printf("signaler.cpp have fork Child process %d signaler_t::wait returning simulating interrupt #1\n", getpid());
        errno = EINTR;
        return -1;
    }
#endif
    ...
    printf("signaler.cpp wait based on poll rc=%d\n", rc);
    if (unlikely (rc < 0)) {
    ...
    if (unlikely (pid != getpid ())) {
        // we have forked and the file descriptor is closed. Emulate an interrupt
        // response.
        printf("singnaler.cpp have fork Child process %d signaler_t::wait returning simulating interrupt #2\n", getpid());
        errno = EINTR;
        return -1;
    }
#endif
    ...
    printf("signaler.cpp wait else select rc=%d\n", rc);
    if (unlikely (rc < 0)) {
        errno_assert (errno == EINTR);
        return -1;
    }
...
#if defined ZMQ_HAVE_EVENTFD
    uint64_t dummy;
    //printf("signaler.cpp recv entry");
    ssize_t sz = read (_r, &dummy, sizeof (dummy));
    printf("signaler.cpp recv read sz=%d sizeof(dummy)=%d\n", sz, sizeof(dummy));
    //printf("_r[0x%x] sz=%d", &_r, sz);
...
#if defined ZMQ_HAVE_EVENTFD
    uint64_t dummy;
    //printf("signaler.cpp recv_failable entry");
    ssize_t sz = read (_r, &dummy, sizeof (dummy));
    //printf("_r[0x%x] sz=%d", &_r, sz);
    printf("signaler.cpp recv_failable fd=%d sz=%d sizeof(dummy)=%d\n", _r, sz, sizeof(dummy));
    if (sz == -1) {
...
#else
    ssize_t nbytes = ::recv (_r, &dummy, sizeof (dummy), 0);
    printf("recv_failable ::recv nbytes=%d\n",nbytes);
    if (nbytes == -1) {
        if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
            errno = EAGAIN;
            return -1;
