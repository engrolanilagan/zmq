...
void *zmq_ctx_new (void)
{
    ...
    printf("zmq.cpp zmq_ctx_new entry\n");
    if (!zmq::initialize_network ()) {
        return NULL;
    }
    printf("zmq.cpp zmq_ctx_new intialize_network okay\n");
    ...
    printf("zmq.cpp zmq_ctx_new ctx ctx_t construction okay \n");
    return ctx;
}
...
static zmq::socket_base_t *as_socket_base_t (void *s_)
{
    ...
    printf("zmq.cpp as_socket_base_t socket?=%d threadSafe?=%d\n", (int)s->check_tag(), (int)s->is_thread_safe());
    if (!s_ || !s->check_tag ()) {
        errno = ENOTSOCK;
	printf("zmq.cpp as_socket_base_t ENOTSOCK\n");
        return NULL;
    }
    return s;
}
...
int zmq_bind (void *s_, const char *addr_)
{
    zmq::socket_base_t *s = as_socket_base_t (s_);
    if (!s){
	printf("zmq.cpp zmq_bind: invalid socket");
        return -1;
    }
    return s->bind (addr_);
}
...
static int s_recvmsg (zmq::socket_base_t *s_, zmq_msg_t *msg_, int flags_)
{
    ...
    printf("zmq.cpp s_recvmsg s->recv rc=%d\n", rc);
    if (unlikely (rc < 0))
        return -1;
...
int zmq_recv (void *s_, void *buf_, size_t len_, int flags_)
{
    ...
    errno_assert (rc == 0);
    printf("zmq.cpp zmq_recv zmq_msg_init successful rc=%d\n", rc);
    const int nbytes = s_recvmsg (s, &msg, flags_);
    printf("zmq.cpp zmq_recv s_recvmsg nbytes=%d\n", nbytes);
