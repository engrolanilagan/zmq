...
int zmq::socket_base_t::setsockopt (int option_,
                                    ...
                                    size_t optvallen_)
{
    printf("socket_base.cpp setsockopt entry option_=%d\n", option_);
    scoped_optional_lock_t sync_lock (_thread_safe ? &_sync : NULL);
    if (unlikely (_ctx_terminated)) {
	printf("socket_base.cpp _ctx_terminated\n");
        errno = ETERM;
        return -1;
    }
    ...
    printf("socket_base.cpp xsetsockopt rc=%d\n", rc);
    ...
    printf("socket_base.cpp options.setsockopt rc=%d\n",rc);
...
int zmq::socket_base_t::bind (const char *endpoint_uri_)
{
    ...
    if (unlikely (_ctx_terminated)) {
	printf("socket_base.cpp bind ETERM\n");
        errno = ETERM;
        return -1;
    }
    ...
    if (unlikely (rc != 0)) {
	printf("socket_base.cpp bind process_commands rc=%d\n",rc);
        return -1;
    }
    ...
    std::string protocol;
    std::string address;
    int val_parse_uri = parse_uri (endpoint_uri_, protocol, address);
    int val_check_protocol = check_protocol (protocol);
    printf("socket_base.cpp bind parse_uri=%d check_protocol=%d\n",val_parse_uri, val_check_protocol);
    if (val_parse_uri || val_check_protocol) {
	//printf("bind parse_uri=%d check_protocol=%d\n",val_parse_uri, val_check_protocol);
        return -1;
    }
...
int zmq::socket_base_t::recv (msg_t *msg_, int flags_)
{
    ...
    //  Check whether the context hasn't been shut down yet.
    if (unlikely (_ctx_terminated)) {
	printf("socket_base.cpp recv exit due to _ctx_terminated\n");
        errno = ETERM;
        return -1;
    }
    ...
    //  Check whether message passed to the function is valid.
     if (unlikely (!msg_ || !msg_->check ())) {
	printf("socket_base.cpp recv exit due to msg check\n");
        errno = EFAULT;
        return -1;
    }
    ...
    //  Get the message.
    printf("socket_base.cpp recv checks done. calling xrecv \n");
    int rc = xrecv (msg_);
    printf("socket_base.cpp recv xrecv done. rc=%d \n", rc);
    ...
    printf("socketbase.cpp recv dontWait=%d rcvtimeo=%d\n", (flags_ & ZMQ_DONTWAIT), options.rcvtimeo);
    if ((flags_ & ZMQ_DONTWAIT) || options.rcvtimeo == 0) {
        ...
        rc = xrecv (msg_);
	printf("socketbase.cpp recv dontWait rc=%d\n", rc);
        if (rc < 0) {
            return rc;
        }
        extract_flags (msg_);
    ...
    const uint64_t end = timeout < 0 ? 0 : (_clock.now_ms () + timeout);
    printf("socket_base.cpp recv timeout=%d end=%ld\n", timeout, end);
    //  In blocking scenario, commands are processed over and over again until
    //  we are able to fetch a message.
    printf("socket_base.cpp recv check _ticks\n");
    //sleep(2);
    //printf("socket_base.cpp recv slept for 2 seconds");
    bool block = (_ticks != 0);
    printf("socket_base.cpp recv checking block=%d and _ticks=%d\n", block, _ticks);
    //sleep(2);
    printf("socket_base.cpp recv entering loop now.\n");
    while (true) {
	printf(".");
...
void zmq::socket_base_t::start_reaping (poller_t *poller_)
{
    ...
    printf("socket_base_t::start_reaping\n");
    _poller = poller_;
...
{
    printf("socket_base.cpp process_commands timeout=%d throttle=%d\n", timeout_, throttle_);
    if (timeout_ == 0) {
    ...
    command_t cmd;
    printf("socket_base.cpp process_commands checking for commands\n");
    int rc = _mailbox->recv (&cmd, timeout_);
    //printf("socket_base.cpp process_commands checked for commands rc=%d\n", rc);

    //  Process all available commands.
    printf("socket_base.cpp process_commands entering while loop rc=%d\n", rc);
    while (rc == 0) {
        cmd.destination->process_command (cmd);
        rc = _mailbox->recv (&cmd, 0);
	printf("socket_base.cpp process_commands _mailbox->recv rc=%d\n", rc);
    }
    printf("socket_base.cpp process_commands exit while loop rc=%d\n", rc);
    if (errno == EINTR)
        return -1;
